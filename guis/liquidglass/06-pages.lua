    ...
    actionCard(quick,'Diagnostics','Inspect Uranium health',1,function() state.Page='Diagnostics'; if state.RenderPage then state.RenderPage() end end)
    ...
    actionCard(quick,'Actions','All Uranium commands',4,function() state.Page='Actions'; if state.RenderPage then state.RenderPage() end end)
...
local function renderSettings()
    ...
    pageTitle.Text='Settings'; pageSubtitle.Text='Liquid Glass, profiles and Uranium'; filterBox.Visible=false
    ...
    sectionHeading(page,'Uranium accent',20)
    ...
    actionCard(page,'Diagnostics','Open Uranium Doctor / diagnostics',101,function() state.Page='Diagnostics'; if state.RenderPage then state.RenderPage() end end)
    ...
    actionCard(page,'Uninject Uranium','Close Uranium and clean up',103,function() pcall(mainapi.Uninject,mainapi) end,true)
end
...
local function diagnosticsText()
    ...
    local loadTrace = shared.UraniumGameLoadTrace
...
local function renderDiagnostics()
    activeOptionRefreshers={}; state.ModuleCards={}; clearChildren(page)
    pageTitle.Text='Diagnostics'; pageSubtitle.Text='Uranium Doctor in Liquid Glass'; filterBox.Visible=false
    ...
    actionCard(page,'Reload game modules','Reinject the current game module set',12,function()
        if mainapi.UpdateGameModules then pcall(mainapi.UpdateGameModules) elseif mainapi.ReloadUranium then pcall(mainapi.ReloadUranium) end
    end)
end
...
local function renderErrors()
    activeOptionRefreshers={}; state.ModuleCards={}; clearChildren(page)
    pageTitle.Text='Error Logs'; pageSubtitle.Text='Errors recorded during this Uranium session'; filterBox.Visible=false
...
local function installedRef()
    if type(shared.UraniumPublicRef)=='string' and shared.UraniumPublicRef~='' then return shared.UraniumPublicRef end
...
local function renderUpdates()
    ...
    textPanel(page,table.concat({
        'Uranium '..tostring(mainapi.Version or '?'),
        'Installed source: '..installedRef(),
        'Branch: main',
        shared.updated and 'An update was applied this session' or 'No pending session update notice'
    },'\n'),1)
    actionCard(page,'Update / reload Uranium','Reload from the latest configured public source',10,function() if mainapi.ReloadUranium then pcall(mainapi.ReloadUranium) end end)
...
local function actionDescription(action)
    ...
    if n:find('update',1,true) then return 'Manage or apply Uranium updates' end
    if n:find('reload',1,true) then return 'Reload the current Uranium runtime' end
    if n:find('profile',1,true) then return 'Switch the active Uranium profile' end
    if n:find('gui',1,true) then return 'Change Uranium interface' end
    if n:find('uninject',1,true) then return 'Close Uranium and clean its hooks' end
    return tostring(action.Category or 'Uranium command')
end
...
local function renderActions()
    activeOptionRefreshers={}; state.ModuleCards={}; clearChildren(page)
    pageTitle.Text='Actions'; pageSubtitle.Text='Uranium commands in one place'; filterBox.Visible=true
