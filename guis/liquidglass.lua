-- Uranium Liquid Glass frontend entry.
...
local function currentRef()
	if type(shared.UraniumPublicRef) == 'string' and shared.UraniumPublicRef:gsub('%s+', '') ~= '' then
		return shared.UraniumPublicRef:gsub('%s+', '')
	end
...
local function fetch(path, ref)
	if type(shared.UraniumFetchSource) == 'function' then
		local ok, result = pcall(shared.UraniumFetchSource, path, ref)
		if ok and validSource(result) then return result end
	end
	local ok, result = pcall(game.HttpGet, game,
		'https://raw.githubusercontent.com/Korsac2026/3232/'..ref..'/'..path, true)
...
local function getPart(name)
	...
	if not body then error('Uranium Liquid Glass: failed to load '..name, 0) end

	if type(writefile) == 'function' and not cachePart(localPath, body) then
		error('Uranium Liquid Glass: could not cache '..localPath..' - unreadable cache after retries', 0)
	end
	return body
end
...
local function loadFeatureMetadata()
    ...
    local function fetchFeature(ref)
        local ok, body = pcall(game.HttpGet, game,
            'https://raw.githubusercontent.com/Korsac2026/3232/'..ref..'/profiles/features.json', true)
...
local function featureTagsFor(module)
    ...
    if type(shared.UraniumPremiumModules) == 'table' and shared.UraniumPremiumModules[key] then add('PREMIUM') end
    return tags
end
...
if featureInsert then
	...
	error('Uranium Liquid Glass: module-card section is missing', 0)
end
...
local cache = type(shared.UraniumCompileCache) == 'table' and shared.UraniumCompileCache or nil
local chunk, compileError = cache and cache[source] or nil
if not chunk then
	chunk, compileError = loadstring(source, 'guis/liquidglass/full.lua')
	if not chunk then error('Uranium Liquid Glass compile failed: '..tostring(compileError), 0) end
