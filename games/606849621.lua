...
local loadstring = function(...)
	...
	if err and vape then vape:CreateNotification('Uranium', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/Korsac2026/3232/'..readfile('aetherv2/profiles/commit.txt')..'/'..select(1, path:gsub('aetherv2/', '')), true) end)
...
run(function()
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Uranium', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
