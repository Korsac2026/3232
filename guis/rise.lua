-- Uranium's accent, rgb(190, 115, 255), as the HSV the GUI works in. It is the
...
local getcustomassets = {
	['aetherv2/assets/rise/slice.png'] = 'rbxasset://risesix/slice.png',
	['aetherv2/assets/rise/blur.png'] = 'rbxasset://risesix/blur.png',
...
local function remoteSourceUrl(path)
	return 'https://raw.githubusercontent.com/Korsac2026/3232/'..installedSourceRef()..'/'..
...
local function downloadFile(path, func)
	...
	if not exists then
		...
		local suc, res = pcall(function()
			if type(shared.UraniumFetchSource) == 'function' then return shared.UraniumFetchSource(path, installedSourceRef()) end
...
local function writeFont()
	...
	writefile('aetherv2/assets/rise/risefont.json', httpService:JSONEncode({
		name = 'ProductSans',
		faces = {
			{style = 'normal', assetId = getcustomasset('aetherv2/assets/rise/SF-Pro-Rounded-Light.otf'), name = 'Light', weight = 300},
			{style = 'normal', assetId = getcustomasset('aetherv2/assets/rise/SF-Pro-Rounded-Regular.otf'), name = 'Regular', weight = 400},
			{style = 'normal', assetId = getcustomasset('aetherv2/assets/rise/SF-Pro-Rounded-Medium.otf'), name = 'Medium', weight = 500},
			{style = 'normal', assetId = getcustomasset('aetherv2/assets/rise/Icon-1.ttf'), name = 'Icon1', weight = 600},
			{style = 'normal', assetId = getcustomasset('aetherv2/assets/rise/Icon-3.ttf'), name = 'Icon3', weight = 800}
		}
	}))
	return getcustomasset('aetherv2/assets/rise/risefont.json')
end
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('aetherv2/profiles/'..game.GameId..'.gui.txt') then
		...
		if not guidata then
			guidata = {Categories = {}}
			self:CreateNotification('Uranium', 'Failed to load GUI settings.', 10, 'alert')
			savecheck = false
		end
	...
	if isfile(profilePath) then
		...
		if not savedata then
			...
			self:CreateNotification('Uranium', 'Failed to load '..self.Profile..' profile.', 10, 'alert')
			savecheck = false
		end
...
function mainapi:DownloadRepoConfigs()
	...
	local function finish(text, kind)
		...
		self:CreateNotification('Uranium', text, 8, kind)
	end

	task.spawn(function()
		self:CreateNotification('Uranium', 'Downloading configs from the repo...', 4, 'info')
		local base = 'https://raw.githubusercontent.com/Korsac2026/3232/'..(isfile('aetherv2/profiles/commit.txt') and readfile('aetherv2/profiles/commit.txt') or 'main')..'/configs/'
		...
		for i, v in mainapi.Modules do
			if v.Enabled or table.find(found, i) then
				...
				if interfacebar.Enabled then
					...
					holderline.Image = getcustomasset('aetherv2/assets/rise/slice.png')
