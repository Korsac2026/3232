...
--
do -- Folders
    if not isfolder("aether") then
        makefolder("aether")
    end
    --
    if not isfolder("aether/Configs") then
        makefolder("aether/Configs")
    end
end
--
do -- Library
    ...
    function Library:UpdateConfigList(List, Type)
        for _, File in listfiles("aether/Configs") do
            local FileName = File:gsub("\\", "/"):gsub("aether/Configs/", ""):gsub(".cfg", "")
            --
            if Type == "Remove" then
    ...
    function Library:Window(Options)
        ...
        function Library:CreateWatermark()
            ...
            --
            Library:UpdateWatermark(("<font color='rgb(%d, %d, %d)'>aether</font>  <font color='rgb(%d, %d, %d)'>%s</font> <font size='10'>FPS</font>  %s"):format(R, G, B, R, G, B, "60", os.date("%X")))
            --
            Library:Notify({
                Message = 'You are using aether.',
                Position = "Top Left",
                Delay = 15
            })
            --
                ...
                Library:Connection(RunService.PostSimulation, function()
                    if Library.UI.Initialized and MainWatermark.Visible then
                        ...
                        if (tick() - Watermark.RefreshTick) > Library.UI.WatermarkRefreshRate then
                            Library:UpdateWatermark(("<font color='rgb(%d, %d, %d)'>aether</font>  <font color='rgb(%d, %d, %d)'>%s</font> <font size='10'>FPS</font>  %s"):format(R, G, B, R, G, B, FPS, os.date("%X")))
                            --
...
--
return (getgenv and getgenv().Library) or Library