        ...
        if type(api) == 'table' and type(api.Modules) == 'table' then
            for key, candidate in pairs(api.Modules) do
                if candidate == module then
                    return tostring(rawName) == 'Main' and 'Uranium' or tostring(rawName)
                end
...
local function renderControlPanel(category, api, startOrder)
    ...
    if count > 0 then
        sectionHeading(page, category == 'Uranium' and 'Uranium controls' or (category..' controls'), order)
        order += 1
...
local function renderCategory(category)
    ...
    pageSubtitle.Text=panelApi and 'Uranium controls and features' or ('Modules in '..category)
...
local function renderHome()
    ...
    pageTitle.Text='Home'; pageSubtitle.Text='Uranium at a glance'; filterBox.Visible=false
    ...
    local hi=label(hero,'Uranium',22,true); hi.Size=UDim2.new(1,-150,0,30); hi.Position=UDim2.fromOffset(120,24); hi.ZIndex=114
    local detail=label(hero,(shared.UraniumPremiumAuthorized and 'Premium' or 'Free')..'  •  '..tostring(mainapi.Profile or 'default')..'  •  v'..tostring(mainapi.Version or '?'),10,false,COLORS.Secondary); detail.Size=UDim2.new(1,-150,0,20); detail.Position=UDim2.fromOffset(121,56); detail.ZIndex=114
