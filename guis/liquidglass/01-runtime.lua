-- Uranium Liquid Glass GUI
-- Completely new unified frontend; Uranium's current new GUI is used only as the
...
local function getBackendSource()
    ...
    if type(shared.UraniumFetchSource) == 'function' then
        local ok, result = pcall(shared.UraniumFetchSource, 'guis/new.lua', currentRef())
        if ok then body = result end
    end
    if not validSource(body) then
        local ok, result = pcall(game.HttpGet, game,
            'https://raw.githubusercontent.com/Korsac2026/3232/'..currentRef()..'/guis/new.lua', true)
        if ok then body = result end
    end
    if not validSource(body) then
        error('Uranium Liquid Glass: could not load the Uranium GUI controller backend', 0)
    end
    return body
end
...
if type(mainapi) ~= 'table' then
    ...
    if not backendChunk then error('Uranium Liquid Glass backend compile failed: '..tostring(backendError), 0) end
    mainapi = backendChunk(license)
end
if type(mainapi) ~= 'table' then error('Uranium Liquid Glass backend returned an invalid controller', 0) end
...
if typeof(guiParent) ~= 'Instance' then error('Uranium Liquid Glass: controller GUI root missing', 0) end

local blur = Lighting:FindFirstChild('UraniumLiquidGlassBlur')
if blur and not blur:IsA('BlurEffect') then blur = nil end
if not blur then
    blur = create('BlurEffect', {Name = 'UraniumLiquidGlassBlur', Size = 0}, Lighting)
end
