    Name = 'UraniumLiquidGlass', Size = UDim2.fromScale(1, 1), BackgroundTransparency = 1,
    BorderSizePixel = 0, Visible = false, ZIndex = 100
}, guiParent)
-- Liquid Glass owns the visible Uranium interface. Keep the legacy controller alive, but hide
...
local brandTitle = label(topbar, 'Uranium', 16, true)
...
local profileTier = label(profileButton, shared.UraniumPremiumAuthorized and 'Premium' or 'Free', 9, false, COLORS.Tertiary)
...
local pageSubtitle = label(content, 'Uranium at a glance', 11, false, COLORS.Tertiary)
...
miniPill.Name = 'UraniumLiquidMini'
...
corner(miniOrb, 8)
local miniLabel = label(miniPill, 'Uranium', 12, true)
...
local CATEGORY_ICONS = {
    ...
    Actions = '⋯', Settings = '⚙', Uranium = '◌', Profiles = '◧', Friends = '♡', Targets = '◎'
}
...
local PANEL_ALIASES = {Main = 'Uranium', Profiles = 'Profiles'}
local PANEL_ICONS = {Uranium = '◌', Profiles = '◧', Friends = '♡', Targets = '◎'}

local function categoryApiFor(displayName)
    local wanted = displayName == 'Uranium' and 'Main' or displayName
...
local function collectPanelCategories(moduleCategories)
    ...
    table.sort(result, function(a, b)
        local priority = {Uranium = 1, Profiles = 2, Friends = 3, Targets = 4}
...
local function updateStatus()
    ...
    local tier = shared.UraniumPremiumAuthorized and 'Premium' or 'Free'
