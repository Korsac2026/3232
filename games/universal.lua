...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			...
			local ref = 'main'
			pcall(function()
				local c = readfile('aetherv2/profiles/commit.txt')
				if type(c) == 'string' and c:gsub('%s+', '') ~= '' then
					ref = c:gsub('%s+', '')
				end
			end)
			return game:HttpGet('https://raw.githubusercontent.com/Korsac2026/3232/' .. ref .. '/' .. select(1, path:gsub('aetherv2/', '')), true)
		end)
