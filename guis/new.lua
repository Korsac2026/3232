local license = ... or {}
-- Uranium neon emerald accent
local accent = {Hue = 0.38, Sat = 0.85, Value = 1, Notch = 3}
local mainapi = {
	Categories = {},
	GUIColor = {
		Hue = accent.Hue,
...
local function addTooltip(gui, text, maxWidth)
	...
	gui.MouseEnter:Connect(function(x, y)
		...
		local contentWidth = tooltipSize.X
		...
		if maxWidth and tooltipSize.X + 12 > maxWidth then
			contentWidth = math.max(40, maxWidth - 12)
			fontsize.Width = contentWidth
			local success, wrappedSize = pcall(getfontsize, text, tooltip.TextSize, uipallet.Font)
			fontsize.Width = math.huge
			if success then tooltipSize = wrappedSize end
			tooltip.TextWrapped = true
			tooltip.TextXAlignment = Enum.TextXAlignment.Left
			tooltip.TextYAlignment = Enum.TextYAlignment.Top
		end
		tooltip.Size = UDim2.fromOffset(contentWidth + 12, tooltipSize.Y + 12)
		tooltip.Text = text
		tooltipMoved(x, y)
	end)
...
local function installedSourceRef()
	local sharedRef = type(shared.UraniumPublicRef) == 'string' and shared.UraniumPublicRef:gsub('%s+', '') or ''
...
local function remoteSourceUrl(path, ref)
	...
	return 'https://raw.githubusercontent.com/Korsac2026/3232/'..selectedRef..'/'..
		select(1, path:gsub('^aetherv2/', ''))
end

local function remoteLoaderUrl()
	return 'https://raw.githubusercontent.com/Korsac2026/3232/main/init.lua'
end

local function remoteCommitUrl(ref)
	return 'https://api.github.com/repos/Korsac2026/3232/commits/'..encodeRemoteSource(ref)
end

local function remoteHistoryUrl(limit)
	local requested = math.clamp(tonumber(limit) or 11, 1, 11)
	return 'https://api.github.com/repos/Korsac2026/3232/commits?sha=main&per_page='..tostring(requested)
end
...
local function downloadFile(path, func)
	...
	if not exists then
		...
		local suc, res = pcall(function()
			if type(shared.UraniumFetchSource) == 'function' then
				return shared.UraniumFetchSource(path, installedSourceRef())
			end
...
getcustomasset = function(path)
	...
	if downloaded then
		...
		warn('[Uranium] Failed to cache asset '..tostring(path)..': '..tostring(downloadError))
	end
...
-- Public configs always use the live publishing branch. Uranium's commit pin is only for
-- application code and must not prevent newly moderated configs from appearing.
function configapi.Presets.Base()
	return 'https://raw.githubusercontent.com/Korsac2026/3232/main/configs/'
end

function configapi.Presets.Backend()
	local configured = getgenv and getgenv().UraniumConfigBackend
	if (not configured or configured == '') and isfile(profileFolder..'/configbackend.txt') then
		configured = readfile(profileFolder..'/configbackend.txt')
	end
	if type(configured) ~= 'string' or configured:match('^%s*$') then
		configured = 'https://aether-config-backend.aether-config-backend-plutoxqq.workers.dev'
	end
...
function configapi.Presets.Fetch(file)
	local success, response = pcall(function()
		return game:HttpGet(configapi.Presets.URL(file), true)
	end)
	if success and response and response ~= '404: Not Found' then return response end
	return game:HttpGet(configapi.Presets.Base()..tostring(file)..'?aether='..tostring(os.time()), true)
end
...
function configapi.Presets.FetchList(sort)
	local player = cloneref(game:GetService('Players')).LocalPlayer
	...
	if backendSuccess and backendResponse and backendResponse ~= '404: Not Found' then
		local decoded = select(2, pcall(httpService.JSONDecode, httpService, backendResponse))
		if type(decoded) == 'table' and type(decoded.presets) == 'table' then return backendResponse end
	end

	local response = game:HttpGet(
		'https://api.github.com/repos/Korsac2026/3232/contents/configs/presets.json?ref=main',
		true
	)
...
function configapi.Presets.RememberOwner(file, receipt, result)
	if type(file) ~= 'string' or file == '' or type(receipt) ~= 'table' then return end
	local player = cloneref(game:GetService('Players')).LocalPlayer
	configapi.Presets.Owners[file:lower()] = {
		file = file,
		token = receipt.token,
		submissionId = receipt.id,
		userId = player.UserId,
		displayName = type(result) == 'table' and (result.displayName or result.name) or nil
	}
	pcall(writefile, configapi.Presets.OwnerPath, httpService:JSONEncode(configapi.Presets.Owners))
end

function configapi.Presets.GetOwner(file)
...
do
	...
	pcall(function()
		...
		local body = game:HttpGet('https://raw.githubusercontent.com/Korsac2026/3232/'..commit..'/profiles/features.json', true)
...
components = {
	...
	ColorSlider = function(optionsettings, children, api)
		local optionapi = {
			Type = 'ColorSlider',
			-- Colour options default to the Uranium accent, the same colour the GUI
	...
	TextList = function(optionsettings, children, api)
		...
		optionsettings.Color = optionsettings.Color or Color3.fromRGB(0, 255, 140)
		...
		function optionapi:Toggle()
			...
			xpcall(function()
				...
				if shared.VapeDeveloper then
					mainapi:CreateNotification('Uranium', 'gui error: '.. err, 15, 'warning')
	...
	if inputService.TouchEnabled then
		if enabled then
			if not mobileBlur or not mobileBlur.Parent then
				...
				mobileBlur.Name = 'UraniumMobileBlur'
...
function mainapi:CreateGUI()
	...
	wordmark.Name = 'UraniumLogo'
	wordmark.Size = UDim2.fromOffset(88, 29)
	wordmark.Position = UDim2.fromOffset(14, 8)
	wordmark.BackgroundTransparency = 1
	wordmark.Text = 'URANIUM'
	wordmark.TextColor3 = Color3.fromRGB(0, 255, 140)
	wordmark.TextSize = 18
	wordmark.FontFace = Font.fromEnum(Enum.Font.GothamBold)
	wordmark.TextXAlignment = Enum.TextXAlignment.Left
	wordmark.Parent = window

	local v2badge = Instance.new('TextLabel')
	v2badge.Name = 'V2Badge'
	v2badge.Size = UDim2.fromOffset(26, 16)
	v2badge.Position = UDim2.fromOffset(104, 14)
	v2badge.BackgroundColor3 = Color3.fromRGB(0, 45, 25)
	v2badge.BackgroundTransparency = 0.2
	v2badge.Text = 'v2'
	v2badge.TextColor3 = Color3.fromRGB(0, 255, 140)
	v2badge.TextSize = 11
	v2badge.FontFace = Font.fromEnum(Enum.Font.GothamBold)
	v2badge.Parent = window
	addCorner(v2badge, UDim.new(0, 4))
	local badgestroke = Instance.new('UIStroke')
	badgestroke.Color = Color3.fromRGB(0, 255, 140)
	badgestroke.Transparency = 0.65
	badgestroke.Thickness = 1
	badgestroke.Parent = v2badge
	...
	addCorner(topline, UDim.new(0, 10))
	local topgrad = Instance.new('UIGradient')
	topgrad.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 255, 140)),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(60, 255, 200)),
	...
	discordbutton.Size = UDim2.fromOffset(0, 0)
	...
	settingsversion.Text = 'Uranium '..mainapi.Version..' '..(
	...
	function categoryapi:CreateGUISlider(optionsettings)
		...
		function optionapi:Load(tab)
			...
			if self.Rainbow or tab.CustomColor then
				...
				-- Preset accents are stored as a notch index, but each GUI theme has
				-- its own notch palette, so re-applying the notch on load turned a
				-- saved accent into a different colour when switching GUIs. Keep the
				-- notch when it still maps to the stored colour (same GUI); otherwise
				-- restore the exact saved HSV so the accent survives the switch.
	...
	discordbutton.MouseButton1Click:Connect(function() end)
...
function mainapi:CreateCategoryList(categorysettings)
	...
	categorysettings.Color = categorysettings.Color or Color3.fromRGB(0, 255, 140)
	...
	if categorysettings.Profiles then
		...
		dl.Size = UDim2.fromOffset(520, 430)
		dl.Position = UDim2.new(0.5, -260, 0.5, -215)
		...
		-- getgenv().UraniumConfigBackend or profiles/configbackend.txt.
		...
		local reviewAccounts = {aetherv2owner = true, Korsac2026qq = true}
		...
		local function responseMessage(response, fallback)
			if type(response) == 'table' then
				return tostring(response.error or (type(response.details) == 'string' and response.details) or fallback)
			end
		...
		local function submitActiveConfig(details, updatePreset)
			...
			if type(config) ~= 'table' then
				mainapi:CreateNotification('Configs', 'The active config JSON is invalid.', 7, 'alert')
				return false
			end
			local guiData
			local guiPath = 'aetherv2/profiles/'..game.GameId..'.gui.txt'
			if isfile(guiPath) then guiData = select(2, pcall(httpService.JSONDecode, httpService, readfile(guiPath))) end
			local owner = updatePreset and configapi.Presets.GetOwner(tostring(updatePreset.file))
			if updatePreset and not owner then
		...
		local fields = {}
		local function submitField(key, placeholder, y, height, limit)
			local box = Instance.new('TextBox')
			...
			box.ClearTextOnFocus, box.MultiLine, box.TextWrapped, box.Parent = false, height > 34, height > 34, submitWindow
		...
		local function validLength(text, maximum)
			local success, length = pcall(utf8.len, text)
		...
		confirmSubmit.MouseButton1Click:Connect(function()
			...
			if submitModePreset and changelog == '' then
				mainapi:CreateNotification('Configs', 'Describe what changed before submitting an update.', 7, 'alert')
				return
			end
			if submitActiveConfig({
		...
		-- Poll each private receipt until moderation completes. Accepted v1 receipts are
		-- retained separately as proof that only the original submitter may create v2+.
		task.spawn(function()
			local receiptPath = profileFolder..'/configsubmissions.json'
			...
			for _, receipt in receipts do
				...
				if ok and type(result) == 'table' and result.status ~= 'pending' then
					if result.status == 'accepted' and result.submissionType == 'new' and result.publishedFile then
						configapi.Presets.RememberOwner(tostring(result.publishedFile), receipt, result)
					end
		...
		local function refreshReviews()
			...
			for _, submission in submissions do
				...
				reason.BackgroundColor3, reason.TextColor3, reason.PlaceholderColor3 = color.Light(uipallet.Main, 0.04), uipallet.Text, color.Dark(uipallet.Text, 0.35)
		...
		local function formatDate(value)
			...
			return year and (day..' '..(months[tonumber(month)] or month)..' '..year) or 'date unknown'
		end
		local function compactCount(value)
			local number = math.max(0, tonumber(value) or 0)
			local function rounded(divisor, suffix)
				return string.format('%.1f', number / divisor):gsub('%.0$', '')..suffix
			end
			if number >= 1000000 then return rounded(1000000, 'm') end
			if number >= 1000 then return rounded(1000, 'k') end
		...
		detailsDelete.Visible = localPlayer.Name:lower() == 'Korsac2026q'
		...
		detailsDelete.MouseButton1Click:Connect(function()
			...
			deleteText.Text = 'This permanently removes '..tostring(deleteTarget.name)..' from Public Configs and deletes '..tostring(deleteTarget.file)..' from GitHub. This cannot be undone from Uranium.'
		...
		function refreshRows()
			for _, entry in rowbuttons do
				local installed = isfile(getConfigPath(entry.Name))
				entry.Button.Text = installed and 'Redownload' or 'Download'
				entry.Button.BackgroundColor3 = Color3.fromHSV(mainapi.GUIColor.Hue, mainapi.GUIColor.Sat, mainapi.GUIColor.Value)
				entry.Button.TextColor3 = mainapi:TextColor(mainapi.GUIColor.Hue, mainapi.GUIColor.Sat, mainapi.GUIColor.Value)
			end
		end
		...
		local function makeRow(preset)
			...
			local function mkbtn(text, xoff, width, y, colour, fn)
				...
				b.Position = UDim2.new(1, xoff, 0, y)
		...
		function refresh()
			...
			task.spawn(function()
				local ok, res = pcall(function()
					return configapi.Presets.FetchList(sortModes[sortIndex].Key)
				end)
				refreshing = false
				if not ok or not res or res == '404: Not Found' then
		...
		syncbutton.MouseButton1Click:Connect(function()
			...
			task.spawn(function()
				local ok, res = pcall(function()
					return configapi.Presets.Fetch(entry.file)
				end)
				local suc, result
...
function mainapi:CreateWelcome()
	...
	for index, entry in {
		...
		{Heading = 'Configs & Updates', Body = 'Loaded from Korsac2026/3232 repository'}
	} do
		local row = Instance.new('Frame')
		row.Name = 'Row'..index
	...
	local function makePresetRow(preset, order)
		...
		get.MouseButton1Click:Connect(function()
			...
			task.spawn(function()
				local ok, res = pcall(function()
					return configapi.Presets.Fetch(tostring(preset.file))
				end)
				local suc, result
				if ok and res and res ~= '404: Not Found' then
	...
	local function fetchPresets()
		...
		task.spawn(function()
			local ok, res = pcall(function()
				return configapi.Presets.FetchList('trending')
			end)
...
function mainapi:CreateSearch()
	...
	local function mobileActive()
		return inputService.TouchEnabled or shared.UraniumMobileMode == true
	end
	...
	function self:SetMobileMode(enabled)
		shared.UraniumMobileMode = enabled and true or false
	...
	self:SetMobileMode(shared.UraniumMobileMode == true)
...
local function createPanel(config)
	...
	function legitapi:CreateModule(modulesettings)
		...
		if modulesettings.Size or moduleapi.ApiCategory == 'Hud' then
			...
			local objectstroke = Instance.new('UIStroke')
			objectstroke.Color = Color3.fromRGB(0, 255, 140)
			objectstroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
function mainapi:CreateOnline()
	...
	logo.BackgroundTransparency = 1; logo.Text = 'U'; logo.TextColor3 = Color3.fromHSV(mainapi.GUIColor.Hue, mainapi.GUIColor.Sat, mainapi.GUIColor.Value)
	logo.TextSize = 14; logo.FontFace = Font.fromEnum(Enum.Font.GothamBold); logo.Parent = window
	local title = Instance.new('TextLabel')
	title.Size, title.Position = UDim2.fromOffset(140, 38), UDim2.fromOffset(36, 0)
	title.BackgroundTransparency, title.Text = 1, 'Uranium Home'
	...
	avatar.Text = 'U'
	avatar.TextColor3 = uipallet.Text
	avatar.TextSize = 26
	avatar.FontFace = Font.fromEnum(Enum.Font.GothamBold)
	...
	welcome.BackgroundTransparency, welcome.Text = 1, 'Welcome to Uranium'
	...
	local sessionRow = row(290, 'Uranium', 'v'..tostring(mainapi.Version)..' • 0m')
...
function mainapi:CreateChangelogs()
	...
	title.Text, title.TextColor3, title.TextSize, title.FontFace = 'Uranium Update Center', uipallet.Text, 14, uipallet.Font
	title.TextXAlignment, title.Parent = Enum.TextXAlignment.Left, window
	local close = addCloseButton(window)
	local status = Instance.new('TextLabel')
	...
	update.MouseButton1Click:Connect(function() if mainapi.ReloadUranium then mainapi.ReloadUranium() end end)
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('aetherv2/profiles/'..game.GameId..'.gui.txt') then
		...
		if not guidata then
			guidata = {Categories = {}}
			self:CreateNotification('Uranium', 'Failed to load GUI settings, Try rejoining ur game', 10, 'alert')
	...
	if isfile(configPath) then
		...
		if not savedata then
			...
			self:CreateNotification('Uranium', 'Failed to load '..self.Profile..' config.', 10, 'alert')
			savecheck = false
		end
...
-- Everything Uranium parks in the shared global tables.
--
-- Uninject cleared shared.vape and left the rest behind, and that is what made another
-- script picked up after a self destruct come back wearing Uranium's skin: every
-- Vape-derived script starts with `shared.vape or _G.vape`, so it adopted this instance's
-- api - and drew itself from Uranium's assets - instead of building its own. The same
...
local claimedGlobals = {
	...
	'ItemOwner',
	'UraniumLoadingScreen',
	'UraniumSetLoadingStatus',
	'UraniumCloseLoadingScreen'
}
local claimedShared = {
	'vape',
	'vapereload',
	...
	for _, root in roots do
		...
		pcall(function()
			for _, child in root:GetChildren() do
				if child:IsA('ScreenGui') and (child:GetAttribute('Uranium') or child.Name == 'UraniumLoading') then
					child:Destroy()
				end
			end
		end)
...
local function reloadUranium(forceBootstrap)
	if reloadInProgress then
		mainapi:CreateNotification('Uranium', 'A reinject is already in progress.', 4, 'warning')
		return
	end
	...
	task.spawn(function()
		...
		if not ok then warn('[Uranium] reload failed:', err) end
	end)
end
mainapi.ReloadUranium = reloadUranium
...
profiles:CreateButton({
	...
	Function = function()
		...
		if suc then
			...
			mainapi:CreateNotification('Uranium', 'Imported config '..result..'.', 5, 'info')
		else
			mainapi:CreateNotification('Uranium', result, 8, 'alert')
		end
	end,
...
general:CreateToggle({
	...
	Function = function(callback)
		...
		if callback and _G.UraniumCloseLoadingScreen then
	...
	Tooltip = 'Prevents Uranium from showing its startup loading screen'
})
...
versionPicker = general:CreateDropdown({
	...
	Function = function(value, mouse)
		...
		mainapi:CreateNotification('Uranium', 'Pinned v'..entry.Version..' ('..entry.Sha:sub(1, 7)..'). Reinjecting…', 6, 'info')
		reloadUranium(true)
	end,
...
general:CreateButton({
	...
	Function = function()
		...
		mainapi:CreateNotification('Uranium', 'Latest version selected. Reinjecting…', 6, 'info')
		reloadUranium(true)
	end,
...
mainapi.MobileMode = general:CreateToggle({
	...
	Function = function(enabled)
		if mainapi.SetMobileMode then mainapi:SetMobileMode(enabled) else shared.UraniumMobileMode = enabled end
	end,
...
general:CreateButton({
	...
	Tooltip = 'Opens Uranium runtime diagnostics'
})
general:CreateButton({
	Name = 'Error Logs',
	Function = function() if mainapi.ErrorLogWindow then mainapi.ErrorLogWindow:Open() end end,
	Tooltip = 'Shows recent errors raised by Uranium modules'
})
general:CreateButton({
	Name = 'Update Center',
...
general:CreateButton({
	...
	Tooltip = 'Removes Uranium from the current game'
})
general:CreateButton({
	Name = 'Reinject',
	Function = reloadUranium,
	Tooltip = 'Reloads Uranium for debugging purposes'
})
local updatingModules = false
local function updateGameModules()
	if updatingModules then
		mainapi:CreateNotification('Uranium', 'A module update is already in progress.', 4, 'warning')
		return
	end
	...
	task.spawn(function()
		local function finish(message, kind)
			...
			mainapi:CreateNotification('Uranium', message, 6, kind)
		end
...
modules:CreateButton({
	...
	Function = function()
		...
		mainapi:CreateNotification('Uranium', 'Module names reset.', 4, 'info')
	end,
...
-- A button rather than a toggle. As a toggle this was a switch you armed and then had to
-- reinject yourself, and the loader consumes the flag on the way past - so it read as
-- enabled long after it had already been spent, and turning it off did nothing at all.
-- There is no state worth keeping here: the action is "load that file, now".
modules:CreateButton({
	Name = 'Force game file',
	Function = function()
		local selected = tostring(forcedGameFile.Value or ''):match('^%s*(%d+)%s*$')
		if not selected then
			mainapi:CreateNotification('Uranium', 'Pick a game file above first.', 6, 'alert')
			return
		end
		writefile('aetherv2/profiles/forcegameid.txt', selected)
		writefile('aetherv2/profiles/forcegame.txt', 'true')
		mainapi:CreateNotification('Uranium', 'Reinjecting with the '..selected..' game file...', 5, 'info')
		reloadUranium()
	end,
	Tooltip = 'Reinjects now using the game file above'
})
...
local captureService = pcall(function() return game:GetService('CaptureService') end) and game:GetService('CaptureService')
if captureService then
	pcall(function()
		captureService.CaptureBegan:Connect(function()
			if mainapi.Streamproof and mainapi.Streamproof.Enabled and gui then
				gui.Enabled = false
			end
		end)
...
local function applyStreamproof(enabled)
	mainapi.StreamproofActive = enabled
	pcall(function() sethiddenproperty(gui, 'VisibleInCapture', not enabled) end)
	pcall(function() gui.VisibleInCapture = not enabled end)
	pcall(function() sethiddenproperty(gui, 'DisplayDesktopUI', not enabled) end)
	-- Oculta tambien los GUIs externos (contador de fly, loadings) que no viven en el gui principal
	pcall(function()
		local containers = {}
		if gethui then
		...
		for _, c in ipairs(containers) do
			if c then
				for _, g in ipairs(c:GetChildren()) do
					if g:IsA('ScreenGui') and (g.Name == 'UraniumFlyCounter' or g.Name == 'UraniumLoading' or g.Name == 'ReaperKeyAuth') then
						g.Enabled = not enabled
						pcall(function() sethiddenproperty(g, 'VisibleInCapture', not enabled) end)
						pcall(function() g.VisibleInCapture = not enabled end)
					end
				end
			end
		end
	...
	if setfflag then
		pcall(setfflag, 'AbuseReportScreenshot', enabled and 'False' or 'True')
		pcall(setfflag, 'DFFlagAbuseReportScreenshot', enabled and 'False' or 'True')
	end
	if enabled then
		if clickgui then clickgui.Visible = false end
		if gui then gui.Enabled = false end
		if mainapi.VapeButton then mainapi.VapeButton.Visible = false end
	else
		if gui then gui.Enabled = true end
		if mainapi.VapeButton and not (isfile('aetherv2/profiles/hide.txt') and readfile('aetherv2/profiles/hide.txt') == 'true') then
			mainapi.VapeButton.Visible = true
		end
		mainapi:UpdateTextGUI()
	end
end

mainapi.Streamproof = guipane:CreateToggle({
	Name = 'Streamproof (Anti-Screenshare)',
	Default = false,
	Function = function(enabled)
		applyStreamproof(enabled)
		if not enabled and mainapi.CreateNotification then
			mainapi:CreateNotification('Uranium', 'Visuals restored', 2, 'info')
		end
	end,
	Tooltip = 'Completely hides the GUI, watermark, and overlays so nothing is visible while streaming or clipping (Hotkey: F7).'
})
...
mainapi:Clean(inputService.InputBegan:Connect(function(input, processed)
	...
	if input.KeyCode == Enum.KeyCode.F7 and not inputService:GetFocusedTextBox() then
		if mainapi.Streamproof then
			mainapi.Streamproof:Toggle()
		end
...
guipane:CreateDropdown({
	...
	Function = function(val, mouse)
		if mouse then
			...
			-- any other unsaved changes carry across the GUI switch.
			pcall(function()
				mainapi:Save(mainapi.Profile)
			end)
	...
	Tooltip = 'new - The current Uranium theme\nold - The pre-v4.05 theme\nrise - Rise 6.0'
})
...
local textguisize = textgui:CreateSlider({
	Name = 'Text size',
	Min = 11,
	Max = 22,
	Default = 15,
...
local textguispacing = textgui:CreateSlider({
	Name = 'Row spacing',
	Min = 0,
	Max = 8,
	Default = 0,
	Suffix = 'px',
	Function = function(value)
		if VapeLabelSorter then VapeLabelSorter.Padding = UDim.new(0, value) end
		mainapi:UpdateTextGUI()
	end
})
local textguialignment = textgui:CreateDropdown({
	Name = 'Alignment',
	List = {'Auto', 'Left', 'Right'},
	Default = 'Auto',
	Tooltip = 'Auto aligns toward the closest screen edge',
	Function = function()
		mainapi:UpdateTextGUI()
	end
})
local textguiextratext = textgui:CreateToggle({
	Name = 'Show module details',
	Default = true,
	Tooltip = 'Shows mode and status text supplied by modules',
	Function = function()
		mainapi:UpdateTextGUI()
	end
})
...
local textguiaccentline = {Enabled = true, Object = {Visible = false}}
local textguibackground = textgui:CreateToggle({
	...
	Function = function(callback)
		...
		textguiaccentline.Object.Visible = callback
		mainapi:UpdateTextGUI()
	end
})
...
textguiaccentline = textgui:CreateToggle({
	Name = 'Accent line',
	Default = true,
	Tooltip = 'Shows the thin GUI-coloured edge beside each module',
	Function = function()
		mainapi:UpdateTextGUI()
	end,
	Darker = true,
...
VapeLogoShadow.Logo2.ImageTransparency = 1
VapeLogoShadow.Logo2.Visible = false
...
VapeLabelSorter = Instance.new('UIListLayout')
...
local function updateTargetInfoPosition(target)
	...
	-- math.clamp errors outright once its bounds invert, which is what a viewport
	-- narrower or shorter than the HUD itself produces. Hide instead.
	if viewport.X < (half.X + margin) * 2 or viewport.Y < (half.Y + margin) * 2 then return false end
...
function mainapi:UpdateTextGUI(afterload)
	...
	if textgui.Button.Enabled then
		local right = textguialignment.Value == 'Right' or (textguialignment.Value == 'Auto' and textgui.Children.AbsolutePosition.X > (gui.AbsoluteSize.X / 2))
		VapeLabelSorter.Padding = UDim.new(0, textguispacing.Value)
		...
		for i, v in mainapi.Modules do
			...
			if v.Enabled or table.find(found, i) then
				...
				holdercolorline.Visible = textguiaccentline.Enabled
...
--[[
	Undo / redo for module toggles (Ctrl+Z / Ctrl+Y).
	Every module toggle you do by hand - clicking a module or pressing its
	keybind - is recorded. Ctrl+Z reverts the most recent toggle and Ctrl+Y
	re-applies it. Config loads and bulk operations never record, so undo only
	ever walks back things you changed manually. Because a toggle is symmetric,
	stepping the stack just re-toggles the same module, which restores its state
	either way.
]]
mainapi.UndoStack = {}
mainapi.RedoStack = {}
function mainapi:PushUndo(module)
	if not module then return end
	table.insert(self.UndoStack, module)
	while #self.UndoStack > 50 do
		table.remove(self.UndoStack, 1)
	end
	table.clear(self.RedoStack)
end
function mainapi:Undo()
	local module = table.remove(self.UndoStack)
	if not module then return end
	module:Toggle()
...
function mainapi:Redo()
	local module = table.remove(self.RedoStack)
	if not module then return end
	module:Toggle()
	table.insert(self.UndoStack, module)
	if self.ToggleNotifications and self.ToggleNotifications.Enabled then
...
mainapi:Clean(inputService.InputBegan:Connect(function(inputObj)
	...
	if inputObj.KeyCode ~= Enum.KeyCode.Z and inputObj.KeyCode ~= Enum.KeyCode.Y then return end
	if not (inputService:IsKeyDown(Enum.KeyCode.LeftControl) or inputService:IsKeyDown(Enum.KeyCode.RightControl)) then return end
...
;(function()
	...
	local function formatErrors()
		...
		return #lines > 0 and table.concat(lines, '\n') or 'No Uranium errors recorded this session.'
	end
	function mainapi:RecordError(moduleName, message)
		...
		if previous and previous.Module == tostring(moduleName or 'Uranium') and previous.Message == message and now - previous.At < 5 then
			previous.Count += 1
			previous.At, previous.Time = now, os.date('%H:%M:%S')
			return
		end
		table.insert(self.ErrorLogs, {Module = tostring(moduleName or 'Uranium'), Message = message, At = now, Time = os.date('%H:%M:%S'), Count = 1})
	...
	local diagnostics = window('UraniumDiagnostics', 'Uranium Diagnostics', UDim2.fromOffset(520, 390))
	...
	local errors = window('UraniumErrorLogs', 'Uranium Error Logs', UDim2.fromOffset(620, 390))
...
;(function()
	...
	function mainapi:GetCommandActions()
		local actions = {
			...
			{Name = 'Update Uranium', Category = 'Action', Action = function() if mainapi.ReloadUranium then mainapi.ReloadUranium() end end},
			{Name = 'Reload game modules', Category = 'Action', Action = function()
				if mainapi.UpdateGameModules then mainapi.UpdateGameModules() else mainapi.ReloadUranium() end
			end},
		...
		for _, guiName in {'new', 'old', 'rise'} do
			table.insert(actions, {Name = 'Change GUI: '..guiName, Category = 'GUI', Action = function()
				...
				if mainapi.ReloadUranium then mainapi.ReloadUranium() end
			end})
		end
		for _, profile in mainapi.Profiles do
